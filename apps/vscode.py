...
context.keymap(
    {
        ...
        "(squash)": Key("cmd-delete"),
        # switch line numberings
        "relative ( numbers | lines )": Key("alt-f4"),
        "(key mode | keyboard)": Key("cmd-alt-ctrl-v"),
        # "key mode": Key("alt-f3"),
        "letter [<dgndictation>]": [Key("alt-/"), text],
        "(next block | empty next)": Key("alt-end"),
        "(next block | empty (preev | previous | back))": Key("alt-home"),
        # Navigating Interface
        "(focus | fox) files": Key("shift-cmd-e"),
        "explore tab": Key("shift-cmd-e"),
        "search (tab | all | files)": [Key("shift-cmd-f"), set_again("f4", "shift-f4")],
        ...
        "(peek reference[s] | find [all] reference[s])": Key("cmd-alt-f7"),
        "fix": Key("cmd-."),
        "what the fuck": Key("cmd-alt-g b"), # git blame
        "fix [this] [<dgndictation>] [over]": [Key("cmd-."), text],
        "fix (all | problems)": Key("cmd-alt-."),
        "(fix lint | lint fix | relax)": Key("cmd-alt-,"), # fix all lint problems
        "cursor up": againKey("alt-cmd-up"),
        "cursor down": againKey("alt-cmd-down"),
        # "slap up[per]": Key("cmd-alt-enter"),
        ...
        "rename file": Key("ctrl-shift-f6"),
        "(untug | right indent)": Key("ctrl-alt-tab"), #todo make outdent shortcut
        # various
        "comment": Key("cmd-/"),
        "(match) (brace | paren)": againKey("cmd-shift-\\"),
        "(ace | care)": Key("alt-;"),
        ...
        "(go mark) [<dgndictation>]": [againKey("alt-\\"), text],
        "(action | palette) [<dgndictation>++] [yes]": [againKey(ACTION_POPUP_KEY), text, press_if("enter", "yes$")],
        "search all [<dgndictation>++] [yes]": [Key("cmd-shift-f"), text, press_if("enter", "yes$")],
        "(complete | drop) [yes]": [Key("ctrl-space"), delay(0.3), press_if("enter", "yes$")],
        
        #vim attempts
        "change ( inside | in ) [<dgndictation>]": [Key('escape c i'), text, set_again('escape .')],
        "change ( with ) [<dgndictation>]": [Key('escape c f'), text, set_again('escape .')],
        "change ( till | until ) [<dgndictation>]": [Key('escape c t'), text, set_again('escape .')],
        "change around [<dgndictation>]": [Key('escape c a'), text, set_again('escape .')],
        "delete ( inside | in ) [<dgndictation>]": [Key('escape d i'), text, set_again('escape .')],
        "delete around [<dgndictation>]": [Key('escape d a'), text, set_again('escape .')],
        "select in (indent | indentation)": [Key('escape g v i i')],
        "select [in] parent [(indent | indentation)]": [Key('escape g v a i')],
        "select ( inside | in ) [<dgndictation>]": [Key('escape v i'), text],
        ...
        # "star": ['*', set_again('n', 'N')],
        # "pound": ['#', set_again('n', 'N')],
        "flesh [<dgndictation>+] [yes]": [Key('escape /'), text, press_if("enter", "yes$"), set_again('escape n', 'escape N')],
        # "sup [<dgndictation>+] [yes]": [Key('escape ?'), text, press_if("enter", "yes$"), set_again('n', 'N')],
        "float [<dgndictation>]": [Key('escape f'), text, set_again('escape ;', 'escape ,')],
        "(unfloat | backflow | back float) [<dgndictation>]": [Key('escape F'), text, set_again(';', ',')],
        # "sneak [<dgndictation>]": [Key('escape s'), text, set_again(';', ',')],
